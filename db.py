...
sql.execute("CREATE TABLE IF NOT EXISTS products(pr_id INTEGER PRIMARY KEY AUTOINCREMENT, pr_name TEXT, pr_price REAL, pr_quantity INTEGER, pr_des TEXT, pr_photo TEXT, reg_date DATETIME);")
...
def check_user(user_id):
    connection = sqlite3.connect('kfc.db')
    sql = connection.cursor()
    # пробуем вытащить пользователя из базы данных
...
def add_product(pr_name, pr_price, pr_quantity, pr_photo, pr_des):
    connection = sqlite3.connect('kfc.db')
    sql = connection.cursor()
    sql.execute("INSERT INTO products(pr_name, pr_price, pr_quantity, pr_des, pr_photo, reg_date) VALUES (?,?,?,?,?,?)", (pr_name, pr_price, pr_quantity, pr_des, pr_photo, datetime.now()))
    connection.commit()
...
def get_exact_product(pr_id):
    connection = sqlite3.connect('kfc.db')
    sql = connection.cursor()
    exact_product = sql.execute("SELECT pr_name, pr_price, pr_des, pr_photo FROM products WHERE pr_id=?;", (pr_id, )).fetchone()
...
def add_to_cart(user_id, pr_id, pr_name, pr_count, pr_price):
    ...
    sql.execute("INSERT INTO cart(user_id, pr_id, pr_name, pr_count, pr_price, total_price) VALUES (?,?,?,?,?,?);", (user_id, pr_id, pr_name, pr_count,pr_price, total_price))

def delete_exact_pr_from_cart(user_id, pr_id):
    connection = sqlite3.connect('kfc.db')
    sql = connection.cursor()
    sql.execute("DELETE pr_id FROM cart WHERE user_id=?;", (pr_id, user_id))
    connection.commit()

def get_user_cart(user_id):
    connection = sqlite3.connect('kfc.db')
    sql = connection.cursor()
    user_cart = sql.execute("SELECT pr_name, pr_count, total_prise FROM cart WHERE user_id=?;", (user_id,)).fetchall()
    return user_cart
